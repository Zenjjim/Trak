...
const MyTasks = ({ myTasks }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  ...
  const [RENAMETHIS, SETRENAMETHIS] = useState<TimeSectionType[]>();
  useEffect(() => {
    SETRENAMETHIS([]);
  }, [router.query]);
  ...
  useEffect(() => {
    const noe = filterAndSearchTasks(searchString, { tags: choosenTags, processTemplates: choosenProcessTemplates }, timeSections);
    SETRENAMETHIS(noe);
      ...
      <div className={classes.root}>
        ...
        <div>
          {!timeSections.length ? (
            ...
            (RENAMETHIS?.length ? RENAMETHIS : timeSections).map((section: TimeSectionType, index: number) => {
